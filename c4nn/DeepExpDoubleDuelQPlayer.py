...
from c4nn.TFSessionManager import TFSessionManager as TFSN
from c4nn.Board import Board, BOARD_SIZE, BOARD_H, BOARD_W, EMPTY, RED, YELLOW
from c4nn.Player import Player, GameResult
...
class QNetwork:
    ...
    def build_graph(self, name: str):
        ...
        with tf.variable_scope(name):
            self.input_positions = tf.placeholder(tf.float32, shape=(None, BOARD_W, BOARD_H, 3), name='inputs')
...
class DeepExpDoubleDuelQPlayer(Player):
    ...
    def board_state_to_nn_input(self, state: np.ndarray) -> np.ndarray:
        ...
        res = res.reshape(3, BOARD_W, BOARD_H)
