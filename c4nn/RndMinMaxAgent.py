...
from c4nn.Board import Board, EMPTY, GameResult
...
class RndMinMaxAgent(Player):
    ...
    def _min(self, board: Board, depth: int = 0) -> int:
        """
        ...
        # if the depth of the minmax reaches the limit, just return a "draw" result.
        elif depth > self.MAX_DEPTH:
            best_moves = {(self.DRAW_VALUE, -1)}
        else:
            #
            ...
            for index in board.valid_moves():
                b = Board(board.state)
                b.move(index, board.other_side(self.side))

                res, _ = self._max(b, depth+1)
    ...
    def _max(self, board: Board, depth: int = 0) -> int:
        """
